...
//@see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		try{			
			...
			sqlQuery="SELECT ti.term as term,CONCAT(ins.first_name,' ',ins.last_name) as instructor,CONCAT(sch.day,' ',sch.start_time,' - ',sch.end_time) as time, CONCAT(bld.name,' ',rm.room_no,' ',bld.campus_short) as location, CONCAT(sbj.subject_code,' ',cs.course_code,' - ',cs.title) as course"; 
			sqlQuery+=" FROM course_details cd, course cs, subject sbj, term_info ti, instructor ins, schedule sch, room rm, building bld"; 
			sqlQuery+=" WHERE ti.id=? and cs.level=? AND";
